...
class MPU6050:
    ...
    def __init__(self, address = MPU6050_DEFAULT_ADDRESS, channel=0):
    ...
    def setFullScaleGyroRange(self, gyroRange):
        self.i2c.writeBits(self.MPU6050_RA_GYRO_CONFIG, self.MPU6050_GCONFIG_FS_SEL_BIT, self.MPU6050_GCONFIG_FS_SEL_LENGTH, gyroRange)        
    ...
    def setTempSensorEnabled(self, enabled):
        ...
        self.i2c.writeBit(self.MPU6050_RA_PWR_MGMT_1, self.MPU6050_PWR1_TEMP_DIS_BIT, value)
    ...
    def getFIFOBlock(self):    
        return self.i2c.readBlock(self.MPU6050_RA_FIFO_R_W)
    ...
    def setDeviceID(self, deviceId):
        self.i2c.writeBits(self.MPU6050_RA_WHO_AM_I, self.MPU6050_WHO_AM_I_BIT, self.MPU6050_WHO_AM_I_LENGTH, deviceId)
    ...
    def dmpGetAccel(self, packet):
        ...
        x = ((packet[28] << 8) + packet[29]) / 8192.0
        y = ((packet[32] << 8) + packet[33]) / 8192.0
        z = ((packet[36] << 8) + packet[37]) / 8192.0
    ...
    def dmpGetLinearAccel(self, accelRaw, gravity):
    
        x = accelRaw[0] - gravity['x']
        y = accelRaw[1] - gravity['y']
        z = accelRaw[2] - gravity['z']
        
        return [x,y,z]
        
    
    def dmpGetLinearAccelInWorld(self, linearAccel, rotationAngles):
        '''
        Rotates accel acording to the angles
        '''
        
        '''
        20170123 DPM: Better calculation of acceleration vector.
        '''
        accel = Vector.rotateVector3D(linearAccel, rotationAngles)
    ...
    def dmpGetEuler(self, q):
        psi = atan2(2.0*q['x']*q['y'] - 2.0*q['w']*q['z'], 2.0*q['w']*q['w'] + 2.0*q['x']*q['x'] - 1.0)
        theta = -asin(2.0*q['x']*q['z'] + 2.0*q['w']*q['y'])
        phi = atan2(2.0*q['y']*q['z'] - 2.0*q['w']*q['x'], 2.0*q['w']*q['w'] + 2.0*q['z']*q['z'] - 1.0)
